      ...
      const create = Effect.fn("SessionProcessor.create")(function* (input: Input) {
        ...
        const handleEvent = Effect.fn("SessionProcessor.handleEvent")(function* (value: StreamEvent) {
          switch (value.type) {
            ...
            case "start-step":
              ctx.snapshot = yield* Effect.promise(() => Snapshot.track())
              yield* session.updatePart({
                id: PartID.ascending(),
                messageID: ctx.assistantMessage.id,
            ...
            case "finish-step": {
              ...
              yield* session.updatePart({
                ...
                snapshot: yield* Effect.promise(() => Snapshot.track()),
                messageID: ctx.assistantMessage.id,
                sessionID: ctx.assistantMessage.sessionID,
                type: "step-finish",
                tokens: usage.tokens,
              ...
              if (ctx.snapshot) {
                const patch = yield* Effect.promise(() => Snapshot.patch(ctx.snapshot!))
