...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
      ...
      const resolveTools = Effect.fn("SessionPrompt.resolveTools")(function* (input: {
        ...
        for (const [key, item] of Object.entries(yield* Effect.promise(() => MCP.tools()))) {
        ...
        const resolvePart: (part: PromptInput["parts"][number]) => Effect.Effect<Draft<MessageV2.Part>[]> = Effect.fn(
          ...
          if (part.type === "file") {
            if (part.source?.type === "resource") {
              ...
              const exit = yield* Effect.promise(() => MCP.readResource(clientName, uri)).pipe(Effect.exit)
              ...
              case "file:": {
                ...
                if (part.mime === "text/plain") {
                  ...
                  if (range.start != null) {
                    ...
                    if (start === end) {
                      const symbols = yield* Effect.promise(() => LSP.documentSymbol(filePathURI)).pipe(
      ...
      const command = Effect.fn("SessionPrompt.command")(function* (input: CommandInput) {
        ...
        const cmd = yield* Effect.promise(() => Command.get(input.command))
        if (!cmd) {
          const available = (yield* Effect.promise(() => Command.list())).map((c) => c.name)
