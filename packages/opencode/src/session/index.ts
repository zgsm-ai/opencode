...
export namespace Session {
  ...
  export const layer: Layer.Layer<Service, never, Bus.Service | Config.Service> = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const updateMessage = <T extends MessageV2.Info>(msg: T): Effect.Effect<T> =>
        Effect.gen(function* () {
          yield* bus.publish(MessageV2.Event.Updated, { info: msg })
          return msg
        }).pipe(Effect.withSpan("Session.updateMessage"))

      const updatePart = <T extends MessageV2.Part>(part: T): Effect.Effect<T> =>
        Effect.gen(function* () {
          yield* bus.publish(MessageV2.Event.PartUpdated, { part: structuredClone(part) })
          return part
  ...
  export const setTitle = fn(z.object({ sessionID: SessionID.zod, title: z.string() }), (input) =>
    runPromise((svc) => svc.setTitle(input)).then(() => get(input.sessionID)),
  )

  export const setArchived = fn(z.object({ sessionID: SessionID.zod, time: z.number().optional() }), (input) =>
    runPromise((svc) => svc.setArchived(input)).then(() => get(input.sessionID)),
  )
