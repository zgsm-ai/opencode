...
describe("Project.fromDirectory", () => {
  test("should handle git repository with no commits", async () => {
    ...
    const costrictFile = path.join(tmp.path, ".git", "costrict")
    const fileExists = await Bun.file(costrictFile).exists()
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  test("should set worktree to root when called from a worktree", async () => {
    ...
    const worktreePath = path.join(tmp.path, "..", `worktree-${suffix}`)
  ...
  test("should accumulate multiple worktrees in sandboxes", async () => {
    ...
    const suffix = path.basename(tmp.path)
    const worktree1 = path.join(tmp.path, "..", `worktree-${suffix}-1`)
    const worktree2 = path.join(tmp.path, "..", `worktree-${suffix}-2`)
