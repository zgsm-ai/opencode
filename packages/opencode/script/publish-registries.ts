...
if (!Script.preview) {
  ...
  const binaryPkgbuild = [
    ...
    "url='https://github.com/zgsm-ai/costrict-cli'",
    "arch=('aarch64' 'x86_64')",
    "license=('MIT')",
    "provides=('opencode')",
    "conflicts=('opencode')",
    "depends=('ripgrep')",
    "",
    `source_aarch64=("\${pkgname}_\${pkgver}_aarch64.tar.gz::https://github.com/zgsm-ai/costrict-cli/releases/download/v\${pkgver}\${_subver}/costrict-cli-linux-arm64.tar.gz")`,
    `sha256sums_aarch64=('${arm64Sha}')`,

    `source_x86_64=("\${pkgname}_\${pkgver}_x86_64.tar.gz::https://github.com/zgsm-ai/costrict-cli/releases/download/v\${pkgver}\${_subver}/costrict-cli-linux-x64.tar.gz")`,
  ...
  const sourcePkgbuild = [
    ...
    "",
    `source=("opencode-\${pkgver}.tar.gz::https://github.com/zgsm-ai/costrict-cli/archive/v\${pkgver}\${_subver}.tar.gz")`,
    `sha256sums=('SKIP')`,
    "",
    "build() {",
    `  cd "opencode-\${pkgver}"`,
    ...
    `  COSTRICT_CHANNEL=latest COSTRICT_VERSION=${pkgver} bun run ./script/build.ts --single`,
    "}",
    "",
    "package() {",
    ...
    "    fi",
    "  fi",
    '  bin="dist/costrict-cli-linux-${target_arch}${base}${libc}/bin/costrict-cli"',
    '  if [ ! -f "$bin" ]; then',
    '    printf "unable to find binary for %s%s%s\\n" "$target_arch" "$base" "$libc" >&2',
    "    return 1",
    "  fi",
    '  install -Dm755 "$bin" "${pkgdir}/usr/bin/costrict-cli"',
    "}",
    "",
  ].join("\n")
  ...
  const homebrewFormula = [
    ...
    `  homepage "https://github.com/zgsm-ai/costrict-cli"`,
    ...
    `      url "https://github.com/zgsm-ai/costrict-cli/releases/download/v${Script.version}/costrict-cli-darwin-x64.zip"`,
    `      sha256 "${macX64Sha}"`,
    "",
    "      def install",
    ...
    "    end",
    "    if Hardware::CPU.arm?",
    `      url "https://github.com/zgsm-ai/costrict-cli/releases/download/v${Script.version}/costrict-cli-darwin-arm64.zip"`,
    `      sha256 "${macArm64Sha}"`,
    "",
    "      def install",
    '        bin.install "bin/costrict-cli"',
