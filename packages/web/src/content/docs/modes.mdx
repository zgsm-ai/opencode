...
:::caution
Modes are now configured through the `agent` option in the costrict-cli config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in costrict-cli allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the costrict-cli config.
...
costrict-cli comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.costrict-cli/plans/*.md` to detail the plan itself
...
- Global: `~/.config/costrict-cli/mode/`
- Project: `.costrict-cli/mode/`

```markdown title="~/.config/costrict-cli/mode/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
If no temperature is specified, costrict-cli uses model-specific defaults (typically 0 for most models, 0.55 for Qwen models).
...
both the global costrict-cli config and the project specific config.
...
Create mode files in `.costrict-cli/mode/` for project-specific modes or `~/.config/costrict-cli/mode/` for global modes:

```markdown title=".costrict-cli/mode/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/costrict-cli/mode/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
