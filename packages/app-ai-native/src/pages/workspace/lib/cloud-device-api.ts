...
export async function checkRuntimeConfig(deviceId: string): Promise<{
  ...
  return withClusterRetry(deviceId, async () => {
    try {
      ...
      if (STALE_ROUTE_RE.test(msg)) throw err
...
export const deviceFileApi = {
  ...
  getConfig: async (deviceId: string): Promise<RuntimeConfig> =>
    withClusterRetry(deviceId, async () => client(deviceId).runtime.config()),
